...
atoms  = Bio.PDB.Selection.unfold_entities(structure[0][proteinchain], "A")
...
for residue in residues_nearby:
    ...
    if not os.path.exists(res_folder):
        ...
        print(f"Ignoring {res_folder} as it already exists")
        continue
    os.chdir(res_folder)
...
#    shutil.copyfile("../3JD.json", "./molecules/3JD.json")
    ...
    for item in (mutation_list):
        with open(f"individual_list_{item}.txt", "w") as text_file:
            text_file.write(item + ";")

        buildmodel = f"{foldxdir}/foldx_20270131 --command=BuildModel --pdb=pdbfile.pdb --numberOfRuns=1 --pH=7 --vdwDesign=2 --ionStrength=0.05 --water=CRYSTAL --order=_USERDEFINED --mutant-file=individual_list_{item}.txt"
        os.system(buildmodel)
        os.rename("WT_pdbfile_1.pdb", item + "_WT_pdbfile.pdb")
        os.rename("pdbfile_1.pdb", item + "_MUT_pdbfile.pdb")
        
        analysecomplex_WT = f"{foldxdir}/foldx_20270131 --command=AnalyseComplex --pdb={item}_WT_pdbfile.pdb --analyseComplexChains={proteinchain},{ligandchain} --pH=7 --vdwDesign=2 --ionStrength=0.05 --water=CRYSTAL"	    	    
        os.system(analysecomplex_WT)
        analysecomplex_MUT = f"{foldxdir}/foldx_20270131 --command=AnalyseComplex --pdb={item}_WT_pdbfile.pdb --analyseComplexChains={proteinchain},{ligandchain} --pH=7 --vdwDesign=2 --ionStrength=0.05 --water=CRYSTAL"
