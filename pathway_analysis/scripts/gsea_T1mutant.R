...
#
# Purpose: GSEA on DESeq2 results for KCNA5 T1 domain mutants vs WT
#          Gene sets: Hallmark
...
# Load DESeq2 results
# -----------------------------------------------------------------------------
res_df <- read.csv("results/deseq2/DESeq2_KCNA5_T1Mutant_vs_WT.csv")
...
run_gsea <- function(gene_ranks,
                      ...
                      outdir,
                      n_top    = 20,
                      subtitle = "TCGA-COAD | T1 mutant (residues 128-216) vs WT") {
    ...
    if (nrow(gsea_sig) > 0) {
        ...
        plot_df <- gsea_sig %>%
            ...
            head(n_top) %>%
            mutate(
                pathway_clean = gsub(paste0("HALLMARK"),
                                      "", pathway),
